...
public class SeccionCritica {
     ...
     */
    public SeccionCritica(int tamanio, Ventana ventanita) {
        ...
        RespuestaFinal = null;
        ventanaPrincipal = ventanita;
        //ventanaPrincipal.jta_Notificaciones.append("Iniciamos Seccion Critica");
    }    
    ...
    public void BuscaArchivo(String nombreHilo, String nombreArchivo) {
        try {
            ...
            //Definimos el Nodo Anterior
            String NodoAnterior="";
            if (HilosUsados == 0) {
                NodoAnterior="Nodo" + (MaximoHilos - 1) + " - " + NombreHilos.get(MaximoHilos - 1);
                ventanaPrincipal.NodoAnterior.setText(NodoAnterior);
                //ventanaPrincipal.NodoAnterior.setText("Nodo" + (MaximoHilos - 1) + " - " + NombreHilos.get(MaximoHilos - 1));
            } else {
                NodoAnterior="Nodo" + (HilosUsados - 1) + " - "+NombreHilos.get(HilosUsados - 1);
                ventanaPrincipal.NodoAnterior.setText(NodoAnterior);
                //ventanaPrincipal.NodoAnterior.setText("Nodo" + (HilosUsados - 1) + " - "+NombreHilos.get(HilosUsados - 1));
            }
            
            //Definimos el Nodo Siguiente
            String NodoSiguiente="";
            if(HilosUsados == MaximoHilos - 1){
                NodoSiguiente="Nodo" + (0) + " - "+NombreHilos.get(0);
                ventanaPrincipal.NodoSiguiente.setText(NodoSiguiente);
                //ventanaPrincipal.NodoSiguiente.setText("Nodo" + (0) + " - "+NombreHilos.get(0));
            }else{
                NodoSiguiente="Nodo" + (HilosUsados + 1) + " - "+NombreHilos.get(HilosUsados + 1);
                ventanaPrincipal.NodoSiguiente.setText(NodoSiguiente);
                //ventanaPrincipal.NodoSiguiente.setText("Nodo" + (HilosUsados + 1) + " - "+NombreHilos.get(HilosUsados + 1));
            }

            //Definimos el Nodo Actual, en este caso la carpeta
            ventanaPrincipal.CarpetaAsociada.setText(nombreHilo);
            
            //Creamos una ventana para cada Hilo
            //Ventana ventanaHilo = new Ventana();
            ...
            ventanaPrincipal.jta_Notificaciones.append("Nodo" + HilosUsados + " - " + nombreHilo + " > " + "Buscando el archivo en:  " + rutaArchivoTemporal + "\n");
            ...
            if (archivito.exists()) {//Si el archivo existe entonces paramos el bucle
                ...
                ventanaPrincipal.jta_Notificaciones.append(nombreHilo + " - " + "Ya lo encontre, ¡PAREN DE BUSCAR! \n\n");
                ...
                if (HilosUsados == MaximoHilos - 1) {//Si se va a llegar al final de los hilos disponibles hacemos lo siguiente
                    ...
                    if (respuestas == JOptionPane.YES_OPTION) {//Reiniciamos la busqueda
                        RespuestaFinal = null;
                        ventanaPrincipal.jta_Notificaciones.setText("");
                        TodosEstanRecorridos = false;
                ...
                ventanaPrincipal.jta_Notificaciones.append(nombreHilo + " - " + "No lo encontre, vas tu: Nodo" + HilosUsados + " - " + NombreHilos.get(HilosUsados) + "\n\n");
    ...
    public void Avisa_Encontrado(String nombreHilo) {
        try {
            ...
            if (posicionFinal >= 0) {
                ...
                ventanaPrincipal.jta_Notificaciones.append("¡EXITO! > Nodo" + HilosUsados + " - " + NombreHilos.get(HilosUsados) + " Aviso a Nodo" + posicionFinal + " - " + NombreHilos.get(posicionFinal) + RespuestaFinal + "\n");
                ...
                if (HilosUsados != 1) {//Mientras no estemos en el nodo de la primera posicion entonces podemos restar, sino se saldra del rango ya que no existe la posicion -1
                    ...
                    ventanaPrincipal.jta_Notificaciones.append("¡EXITO!" + RespuestaFinal + "\n");
    ...
    public void Avisa_NO_Encontrado(String nombreHilo) {
        try {
            ...
            if (posicionFinal >= 0) {
                ...
                ventanaPrincipal.jta_Notificaciones.append("¡FRACASO! > Nodo" + HilosUsados + " - " + NombreHilos.get(HilosUsados) + " Aviso a Nodo" + posicionFinal + " - " + NombreHilos.get(posicionFinal) + RespuestaFinal + "\n");
                ...
                if (HilosUsados != 1) {//Mientras no estemos en el nodo de la primera posicion entonces podemos restar, sino se saldra del rango ya que no existe la posicion -1
                    ...
                    //En ese caso perticular mandamos a que apunte al final de la lista
                    HiloSeleccionado = NombreHilos.get(MaximoHilos - 1);
                    ventanaPrincipal.jta_Notificaciones.append("¡FRACASO!" + RespuestaFinal + "\n");
    ...
    public void HilosVivos(){
        try {
            //Bloqueamos seccion critica
            semaforo.acquire();
            
            int HilosAnteriores = Integer.parseInt(ventanaPrincipal.HilosActivos.getText());
            HilosAnteriores--;
            ventanaPrincipal.HilosActivos.setText(""+HilosAnteriores);
            
            //Liberamos seccion critica
            semaforo.release();
        } catch (InterruptedException e) {
            System.out.println("Error al actualizar hilos vivos: " + e.getMessage());
        }
    }
